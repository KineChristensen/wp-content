				...
				if ( $post_id == $post['post_id'] )
				{
					foreach( $post['postmeta'] as $meta )
					{
						if ( $key == $meta['key'] )
						{
							...
							//preg_replace( '!s:(\d+):"(.*?)";!es', "'s:'.strlen('$2').':\"$2\";'", stripslashes( $meta['value'] ) );
							$value = maybe_unserialize( preg_replace_callback(
								'!s:(\d+):"(.*?)";!es',
								array( $this, 'preg_replace_fix' ),

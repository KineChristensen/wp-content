...
function wpgmza_update_marker_custom_fields($marker_id, $field_data)
{
	...
	for($i = 0; $i < count($_POST['custom_fields']); $i++)
	{
		...
		$custom_fields->{$field_data['field_id']} = $field_data['value'];
	}
}
	...
	if(isset($map_other_settings['list_markers_by']) && $map_other_settings['list_markers_by'] == '6') {
		...
		if($order_by == 'priority')
		{
			$qstr = "SELECT {$wpdb->prefix}wpgmza.id 
				...
				WHERE map_id = " . (int)$atts['id'] . "
				ORDER BY priority $order_dir";
		}
		else
		{
			$qstr = "SELECT id FROM {$wpdb->prefix}wpgmza WHERE map_id = " . (int)$atts['id'] . " ORDER BY $order_by $order_dir";
		}
...
function wpgmaps_user_javascript_pro($atts = false) {
    ...
    if ($short_code_active == true) {
	    ...
	    if(isset($wpgmza_settings['wpgmza_settings_remove_api']) && $wpgmza_settings['wpgmza_settings_remove_api'] == "yes"){ } else { 
	        ...
	        if( $wpgmza_api_key ){
	            wp_enqueue_script('wpgmza_api_call', '//maps.google'.$wpgmza_suffix.'/maps/api/js?'.$api_version_string.'key='.$wpgmza_api_key.'&language='.$wpgmza_locale.'&libraries=geometry,places,visualization', array(), $wpgmza_pro_version.'p', false );
	        } else {
	            wp_enqueue_script('wpgmza_api_call', '//maps.google'.$wpgmza_suffix.'/maps/api/js?'.$api_version_string.'language='.$wpgmza_locale.'&libraries=geometry,places,visualization', array(), $wpgmza_pro_version.'p', false );            
	        }
    ...
    jQuery(function() {
        ...
        var wpgmza_table_length;
                ...
                jQuery(document).ready(function(){
                    ...
                    jQuery("body").on("click", ".wpgmza_edit_btn", function() {
						...
						(function($) {
							...
							var field_data = $("#wpgmza_hid_marker_custom_fields_json_" + cur_id).val();
							
							if(!field_data)
								return;
							
							var json = JSON.parse(field_data);
							
							for(var name in json)
							{
								name = name.replace(/'/, "'");
								$("input[data-custom-field-name='" + name + "']").val(json[name]);
							}
                        ...
                        if (typeof wpgm_lat !== "undefined" && typeof wpgm_lng !== "undefined" && (wpgm_lat.match(/[a-zA-Z]/g) === null && wpgm_lng.match(/[a-zA-Z]/g) === null) && checker.length === 2 && (checker1 != NaN && (checker1 <= 90 || checker1 >= -90)) && (checker2 != NaN && (checker2 <= 90 || checker2 >= -90))) {
							...
							jQuery("#wpgmaps_tabs_markers input[name^='wpgmza-custom-field-']").each(function(index, el) {
								data.custom_fields.push({
									field_id: parseInt( $(el).attr("name").match(/\d+/)[0] ),
									marker_id: -1,
									value: $(el).val()
								});
							});
           ...
           <?php
			...
			{
				var data = $.extend({}, wpgmza_circle_data_array[circle_id]);
				data.map = MYMAP.map;
			...
			{
				var data = $.extend({}, wpgmza_rectangle_data_array[rectangle_id]);
				data.map = MYMAP.map;
...
function wpgmaps_settings_page_sub($section) {
    ...
    if ($section == "mapsettings" && current_user_can('administrator')) {
        $wpgmza_settings = get_option("WPGMZA_OTHER_SETTINGS");
                    ...
                    <tr>
                        ...
                        <td>
                           <select id='wpgmza_api_version' name='wpgmza_api_version'  >
                               <option value=\"3.30\" ".$wpgmza_api_version_selected[0].">3.30</option>
                               <option value=\"3.31\" ".$wpgmza_api_version_selected[1].">3.31</option>
...
function wpgmaps_head_pro() {
    ...
    if (isset($_POST['wpgmza_savemap'])){
		...
		$other_settings['store_locator_style'] = (!empty($_POST['wpgmza_store_locator_style']) ? $_POST['wpgmza_store_locator_style'] : 'legacy');
		...
		if(isset($_POST['wpgmza_settings_disable_infowindows'])) { $wpgmza_data['wpgmza_settings_disable_infowindows'] = true; } else { $wpgmza_data['wpgmza_settings_disable_infowindows'] = false; } 
...
function wpgmaps_b_admin_edit_heatmap_javascript($mapid,$polyid) {
        ...
        if( get_option( 'wpgmza_google_maps_api_key' ) ){ ?>
	        <script type="text/javascript">
	            var gmapsJsHost = (("https:" == document.location.protocol) ? "https://" : "http://");
	            var wpgmza_api_key = '<?php echo get_option( 'wpgmza_google_maps_api_key' ); ?>';
	            document.write(unescape("%3Cscript src='" + gmapsJsHost + "maps.google<?php echo $wpgmza_suffix; ?>/maps/api/js?v=3.exp&key="+wpgmza_api_key+"&language=<?php echo $wpgmza_locale; ?>&libraries=geometry,places,visualization' type='text/javascript'%3E%3C/script%3E"));
	        </script>
	    <?php } else { ?>
	        <script type="text/javascript">
	            var gmapsJsHost = (("https:" == document.location.protocol) ? "https://" : "http://");
	            document.write(unescape("%3Cscript src='" + gmapsJsHost + "maps.google<?php echo $wpgmza_suffix; ?>/maps/api/js?v=3.exp&language=<?php echo $wpgmza_locale; ?>&libraries=geometry,places,visualization' type='text/javascript'%3E%3C/script%3E"));
	        </script>
	    <?php } ?>

...
class Beautiful_Taxonomy_Filters_Public {
	...
	*/
	...
	public static function get_term_post_count_by_type( $term, $taxonomy, $post_type ) {
		...
		return ( isset( $postcount_query->posts ) && count( $postcount_query->posts > 0 ) ? count( $postcount_query->posts ) : 0 );
	...
	public function update_filters_callback() {
		...
		$sql = '
        SELECT COUNT( DISTINCT wp_posts.ID ) as term_count, terms.term_id as term_id, terms.name as term_name, terms.slug as term_slug, term_taxonomy.taxonomy as taxonomy
        FROM wp_posts
		...
		if ( $selects ) {
			foreach ( $selects as $select ) {
				...
				$sql .= sprintf( ' LEFT JOIN wp_term_relationships as %1$s_term_relationship ON (wp_posts.ID = %1$s_term_relationship.object_id)', $select['taxonomy'] );
		...
		$sql .= sprintf( ' WHERE wp_posts.post_type = \'%s\' AND wp_posts.post_status = \'publish\'', $post_type );
		$sql .= $sql_ands;
		...
		if ( $current_taxonomies ) {
          ...
          $term_object = get_term( $term, $key );
					...
					if ( is_array( $value->rewrite ) && array_key_exists( 'slug', $value->rewrite ) ) {
						$new_url .= $value->rewrite['slug'] . '/' . $term_object->slug . '/';
					} else {
						$new_url .= $key . '/' . $term_object->slug . '/';
					}
				}
			}
		}
